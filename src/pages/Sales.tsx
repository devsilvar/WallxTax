...
const SOURCES = ['bank_transfer', 'paycode', 'pos', 'online_store', 'manual'] as const;
...
function statusBadge(s: string) {
  const m: Record<string, string> = { confirmed: 'bg-green-100 text-green-700', pending: 'bg-yellow-100 text-yellow-700', reversed: 'bg-red-100 text-red-700', disputed: 'bg-orange-100 text-orange-700' };
  return <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-medium capitalize ${m[s] || 'bg-gray-100 text-gray-600'}`}>{s}</span>;
}
...
export default function Sales() {
  ...
  const hasActiveFilters = !!(filterSource || filterStatus || filterStartDate || filterEndDate);

  const clearFilters = () => {
    setFilterSource(''); setFilterStatus(''); setFilterStartDate(''); setFilterEndDate('');
    setPage(1);
  };
  ...
  useEffect(() => { fetchSales(); }, [biz, page, filterSource, filterStatus, filterStartDate, filterEndDate]);
  useEffect(() => { fetchSummary(); }, [biz, summaryMonth, summaryYear]);

  const resetForm = () => {
    setAmount(''); setSource('manual'); setDescription(''); setCustomerName('');
    setTransactionDate(new Date().toISOString().slice(0, 10));
    setEditId(null); setShowForm(false);
  };

  const openEdit = (s: SalesTransaction) => {
    setEditId(s.id); setAmount(String(Number(s.amount))); setSource(s.source);
    setDescription(s.description || ''); setCustomerName(s.customerName || '');
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setSaving(true);
    const body = { amount: Number(amount), source, description: description || undefined, customerName: customerName || undefined, transactionDate };
    try {
      if (editId) {
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 font-body text-sm text-gray-500">Manage your sales transactions.</p>
        </div>
        <Button onClick={() => { resetForm(); setShowForm(true); }} className="self-start sm:self-auto">
      ...
      <Card>
        ...
        {summaryLoading ? (
          ...
          <div className="py-6 text-center">
            ...
            <p className="mt-2 font-body text-sm text-gray-400">No confirmed sales for {monthLabel}.</p>
          </div>
        ) : (
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="rounded-lg bg-green-50 px-4 py-3">
                ...
                <p className="mt-1 text-xl font-bold text-green-700">{formatNaira(Number(summary.totalSales))}</p>
              </div>
              <div className="rounded-lg bg-gray-50 px-4 py-3">
                <p className="font-body text-xs text-gray-500 uppercase tracking-wider">Transactions</p>
            ...
            {summary.sourceBreakdown.length > 0 && (
              <div>
                <p className="mb-2 text-xs font-medium text-gray-500 uppercase tracking-wider">By Source</p>
                {/* Stacked bar */}
                <div className="flex h-3 overflow-hidden rounded-full bg-gray-100">
                  {summary.sourceBreakdown.map((sb) => {
                    const pct = Number(summary.totalSales) > 0
                ...
                <div className="mt-3 grid grid-cols-1 gap-y-2 sm:grid-cols-2 sm:gap-x-6 lg:grid-cols-3">
                  {summary.sourceBreakdown.map((sb) => (
                    <div key={sb.source} className="flex items-center gap-2">
                      <span className={`h-2.5 w-2.5 rounded-full ${SOURCE_COLORS[sb.source] || 'bg-gray-400'}`} />
                      <span className="font-body text-sm text-gray-600 capitalize">{sourceLabel(sb.source)}</span>
                      <span className="ml-auto font-body text-sm font-medium text-gray-700">{formatNaira(Number(sb.total))}</span>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {showForm && (
        <Card>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-semibold text-gray-900">{editId ? 'Edit Sale' : 'New Sale'}</h2>
          ...
          <form onSubmit={handleSubmit} className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-1">
              ...
              <select value={source} onChange={(e) => setSource(e.target.value)} className="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
                {SOURCES.map((s) => <option key={s} value={s}>{s.replace('_', ' ')}</option>)}
              </select>
            </div>
            <Input label="Description" value={description} onChange={(e) => setDescription(e.target.value)} />
            <Input label="Customer Name" value={customerName} onChange={(e) => setCustomerName(e.target.value)} />
            <Input label="Transaction Date" type="date" value={transactionDate} onChange={(e) => setTransactionDate(e.target.value)} required />
          ...
          >
            ...
            {hasActiveFilters && (
              <span className="ml-1 inline-flex h-5 w-5 items-center justify-center rounded-full bg-primary-600 text-xs text-white">
                {[filterSource, filterStatus, filterStartDate, filterEndDate].filter(Boolean).length}
              </span>
            )}
          </button>
          {hasActiveFilters && (
          ...
          <Card className="py-4">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
              <div className="space-y-1">
                ...
                <select value={filterSource} onChange={(e) => { setFilterSource(e.target.value); setPage(1); }} className="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500">
              ...
              <div className="space-y-1">
                ...
                <select value={filterStatus} onChange={(e) => { setFilterStatus(e.target.value); setPage(1); }} className="block w-full rounded-lg border border-gray-300 px-3 py-2 text-sm capitalize focus:outline-none focus:ring-2 focus:ring-primary-500">
      ...
      {!isLoading && sales.length === 0 && (
        <Card className="py-12 text-center">
          ...
          <p className="mt-3 font-body text-sm text-gray-400">No sales found.</p>
        </Card>
      )}
            ...
            <tbody className="font-body text-sm">
              {sales.map((s) => (
                <tr key={s.id} className="border-b border-gray-50 hover:bg-gray-50">
                  <td className="px-4 py-3 text-gray-600">{formatDate(s.transactionDate)}</td>
                  <td className="px-4 py-3 text-gray-700">{s.description || '—'}</td>
                  <td className="px-4 py-3 capitalize text-gray-600">{sourceLabel(s.source)}</td>
                  <td className="px-4 py-3 text-right font-semibold text-gray-900">{formatNaira(Number(s.amount))}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <div className="flex items-center justify-end gap-1">
                      ...
                      <button onClick={() => handleDelete(s.id)} className="rounded p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-500"><Trash2 className="h-4 w-4" /></button>
                    </div>
                  </td>
                </tr>
              ))}
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="font-semibold text-gray-900">{formatNaira(Number(s.amount))}</span>
                    {statusBadge(s.status)}
                  </div>
                  <p className="mt-1 text-sm text-gray-600 truncate">{s.description || '—'}</p>
                  <p className="mt-1 text-xs text-gray-400">
                    {formatDate(s.transactionDate)} · <span className="capitalize">{sourceLabel(s.source)}</span>
                  </p>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => openEdit(s)} className="rounded-lg p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-600"><Pencil className="h-4 w-4" /></button>
                  <button onClick={() => handleDelete(s.id)} className="rounded-lg p-2 text-gray-400 hover:bg-red-50 hover:text-red-500"><Trash2 className="h-4 w-4" /></button>
                </div>
              </div>
            </Card>
          ))}
